---
name: img2txt
description: 统一读取本地路径、file URL、公开网络 URL、Data URL、Base64、一个或多个上传附件或 Windows 剪贴板中的图片，并按固定的 GLM 到 Gemini 模型池返回带模型归属的文字结果。用于看图、描述图片、比较多图、分析截图、解释图表和识别图片文字。用户仅发送图片、附件、图片路径或图片 URL 而没有文字说明时也必须立即使用；当前消息确有图片附件但平台没有提供可读取路径时，必须立即使用受控剪贴板回退；当前 Agent、图片加载器或系统在本回合产生 `Cannot read 'image.png' (this model does not support image input)`、`this model does not support image input`、`Unsupported Image`、图片无法显示、无法加载或无法直接读取等错误时，也必须立即使用 `clipboard-fallback`，不得询问是否调用视觉模型，不得解释模型限制或等待用户回复。没有明确任务时默认详细描述图片。
---
...
- 当前 Agent、图片加载器或系统在本回合产生 `Cannot read '<name>' (this model does not support image input)`、`Unsupported Image`、无法显示或无法加载等图片能力错误时，不回复能力限制、不等待用户确认，立即使用 `clipboard-fallback` 读取当前 Windows 剪贴板一次。
...
2. 按“输入选择顺序”收集所有明确提供的图片。单图执行一次；多图按附件或用户列出的顺序逐张执行，不猜测文件名。当前消息中的真实图片附件缺少可读取路径，或当前 Agent/图片加载器/系统刚产生图片输入不支持错误时，把该项记为受控剪贴板回退。用户自己在普通文字中引用、讨论或转述相同错误不满足该条件。
...
6. 退出码为 `0` 时，使用 stdout 回答用户，并保留末尾 `[识别模型: provider/model]`。受控回退还必须保留 `[图片来源: Windows 剪贴板（图片直读失败回退）]`；多图时记录每张图实际使用的模型和来源，综合回答中明确列出。
...
以下两种当前回合信号触发自动受控回退：平台图片附件没有可读取的真实路径；当前 Agent、图片加载器或系统刚产生图片输入不支持错误。对该项使用 `clipboard-fallback`，只读取当前 Windows 剪贴板一次，并在 stderr 和成功 stdout 中明确标注。普通缺失路径、过去消息中的图片、用户自己输入或粘贴的错误文本、以及无法解码的已有文件都不得触发该回退。
...
聊天中出现的 `image.png` 等显示名不等于可读取路径。当前平台附件缺少路径，或当前 Agent/系统刚产生 `Cannot read '<name>' (this model does not support image input)` 等图片能力错误时，直接使用 `clipboard-fallback`；不要把同名文件当作附件，也不要搜索工作目录。用户只是在普通文字中引用该错误时不读取剪贴板。
...
- 只有三种情况允许读取剪贴板：用户在当前请求中明确指定；当前消息确有图片附件但真实路径缺失；当前 Agent、图片加载器或系统在本回合刚产生图片输入不支持错误。后两种仅授权执行一次 `clipboard-fallback`，不得扩展到普通文件错误、过去消息或用户引用的错误文本。
