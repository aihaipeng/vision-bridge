...
async function requestModel({ model, payload, key, fetchImpl }) {
  ...
  try {
    ...
    throw new ProviderError(PROVIDER, 'NETWORK', `Mistral 网络请求失败: ${singleLine(error.message || error)}`, {
      model, retryable: true, cause: error,
    });
  }
  const raw = await response.text();
  if (response.status === 401 || response.status === 403) {
    throw new ProviderError(PROVIDER, 'AUTH', `Mistral API key 无效、已失效或无权访问（HTTP ${response.status}）`, {
  ...
  if (!response.ok) {
    ...
    throw new ProviderError(PROVIDER, modelNotFound ? 'MODEL_NOT_FOUND' : 'HTTP', `Mistral 模型 ${model} 调用失败（HTTP ${response.status}）: ${singleLine(raw).slice(0, 300)}`, {
  ...
  try { data = JSON.parse(raw); }
    ...
    throw new ProviderError(PROVIDER, 'INVALID_JSON', `Mistral 模型 ${model} 返回了非 JSON 响应`, { model, cause: error });
  }
...
async function describe({ image, prompt, key, models = DEFAULT_MODELS, model, fetchImpl, onStatus, fallbackTarget }) {
  ...
  throw new ProviderError(PROVIDER, 'MODELS_FAILED', `所有 Mistral 模型均失败: ${failures.map((item) => `${item.model}: ${item.message}`).join(' | ')}`, {
    failures,
  });
}
